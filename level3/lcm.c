 ...
 * that value. Then we just jump in to a recursive loop that checks to see if
 * the remainder of n divided by both and b is zero. If it is, it means they're
 * we've found the lowest common multiple! If not, we increment n by one and
