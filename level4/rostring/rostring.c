 ...
 * cofusing and I lost track of shit fast. This is a solution that is, imo,
 * easier to follow. So here we go. */
	...
	while (s[i])
	{
		...
		if (s[i] && !is_whitespace(s[i]))
		{
			if (first_word_length == 0) //if we at the start of the first word
				while (s[i] && !is_whitespace(s[i++])) //and its not whitespace
			...
			else
			{
				...
				while (s[i] && !is_whitespace(s[i]) && write(1, &s[i++], 1));
				write(1, " ", 1);
			}
		}
	}
